...
#define LSIZE 16	/* change LSIZE to 16/32 accordingly */
...
EXTERN int mdstep;		/* # of molecular dynamics step */
...
EXTERN int hmc_it;              /* # of HMC sweeps, counting only 
                                   accepted ones  */
EXTERN int counter;		/* counter for HMC loop */
...
EXTERN double *store;		/* storage space for sigma configuration while
			   	   the code is running. */
EXTERN double *conf;		/* storage space for sigma config. generated
				   in the last accepted loop. */
EXTERN double *con;		/* storage space for sigma config. generated
				   in the last but one loop. */
...
EXTERN double *ac_prop;		/* storage for propagator during autocrln. 
				   calculation loops*/
...
/* vectors gen_pt[lattice_site][pointer_number] for gather routine etc */
/* #define N_POINTERS 1 */
